...
export function ItemCard({ vendor, item }: ItemCardProps) {
  ...
  const saveOrder = useCallback(async (newQty: number, newStock: number) => {
    const now = Date.now()
    const orderData = {
      id: order?.id,
      date: currentDate,
  ...
  }, [currentDate, vendor, item.name, config.staff, config.storeId, order?.id, updateOrder, upsertOrder])
